...
var SidebarController = {
  ...
  setOasisAssistantOverlayOAuthCallbackBaseUrl(url) {
    const normalized =
      typeof url === "string" && /^https?:\/\//i.test(url)
        ? url.replace(/\/+$/, "")
        : null;
    this._oasisOverlayOAuthCallbackBaseUrl = normalized;
    this.applyOasisAssistantOverlayOAuthCallbackBaseUrl();
    return this.getOasisAssistantOverlayOAuthCallbackBaseUrl();
  },

  getOasisAssistantOverlayOAuthCallbackBaseUrl() {
    return this._oasisOverlayOAuthCallbackBaseUrl || "https://kahana.co";
  },

  applyOasisAssistantOverlayOAuthCallbackBaseUrl() {
    const overlayBrowser = document.getElementById(
      "oasis-assistant-overlay-browser"
    );
    if (!overlayBrowser || !this._oasisOverlayOAuthCallbackBaseUrl) {
      return false;
    }

    try {
      const assistantWindow =
        overlayBrowser.contentWindow?.wrappedJSObject ||
        overlayBrowser.contentWindow;
      if (
        assistantWindow &&
        typeof assistantWindow.oasisSetOAuthCallbackBaseUrl === "function"
      ) {
        const applied = assistantWindow.oasisSetOAuthCallbackBaseUrl(
          this._oasisOverlayOAuthCallbackBaseUrl
        );
        console.log(
          "[Sidebar] Applied Oasis overlay OAuth callback base URL:",
          applied
        );
        return true;
      }
      ...
      console.error(
        "[Sidebar] Failed to apply Oasis overlay OAuth callback base URL:",
        error
      );
    }
  ...
  toggle(commandID = this.lastOpenedId, triggerNode) {
    ...
    // Special-case Oasis Assistant overlay: if the overlay is active, treat the toggle as a hide action
    const sidebarInfo = this.sidebars.get(commandID);
    const isOasisAssistantToggle =
      commandID === "viewOasisAssistantSidebar" ||
      (sidebarInfo && sidebarInfo.name === "oasis-assistant");
    if (
      isOasisAssistantToggle &&
      document.documentElement.hasAttribute("oasis-assistant-overlay") &&
      this._state?.command === commandID &&
      this._state?.panelOpen
    ) {
      // Overlay is visible: hide it instead of showing. Use dismissPanel:false so we don't unload the iframe
      // and lose conversation state when the user simply toggles the button.
      this.hide({ triggerNode, dismissPanel: false });
      this.updateToolbarButton();
      return Promise.resolve();
    }
  ...
  async show(commandID, triggerNode) {
    console.log(
      "[Sidebar] show() called with commandID:",
      commandID,
      "type:",
      typeof commandID
    );
  ...
  _show(commandID) {
    console.log(
      "[Sidebar] _show() called with commandID:",
      commandID,
      "type:",
      typeof commandID
    );
    ...
    if (isOasisAssistant && !this._oasisForceSidebar) {
      console.log(
        "[Sidebar] Oasis Assistant detected, attempting to use overlay"
      );
      ...
      console.log("[Sidebar] Overlay elements:", {
        overlay: !!overlay,
      ...
      if (overlay && overlayBrowser) {
        ...
        if (!this._oasisOverlayLoadHandler) {
          this._oasisOverlayLoadHandler = () => {
            this.applyOasisAssistantOverlayOAuthCallbackBaseUrl();
          };
          overlayBrowser.addEventListener(
            "load",
            this._oasisOverlayLoadHandler
          );
        }
        ...
        const handleDragMove = e => {
          ...
          newLeft = Math.max(
            contentBounds.left,
            Math.min(newLeft, contentBounds.right - rect.width)
          );
          newTop = Math.max(
            contentBounds.top,
            Math.min(newTop, contentBounds.bottom - rect.height)
          );
        ...
        this._oasisOverlayMessageHandler = evt => {
          ...
          if (
            ...
            console.log(
              "[Sidebar] Ignoring duplicate button message:",
              data.type
            );
            return;
          }
          ...
          if (
            ...
            console.log(
              "[Sidebar] Window expanded to:",
              overlayShell.style.width,
              overlayShell.style.height
            );
        ...
        console.warn(
          "[Sidebar] Overlay elements not found, falling back to sidebar"
        );
      }
    }
  ...
  hide({ triggerNode, dismissPanel = this.sidebarRevampEnabled } = {}) {
    ...
    if (
      ...
      console.log(
        "[Sidebar] Overlay attribute:",
        document.documentElement.getAttribute("oasis-assistant-overlay")
      );
      this._recordPanelToggle(this.currentID, false);
      ...
      // If this is an explicit dismiss (dismissPanel=true), unload the iframe and cleanup listeners.
      // If this is a temporary hide (e.g., toggling the button), keep the iframe src and message handler so
      // conversation state is preserved and will be restored when the overlay is shown again.
      if (dismissPanel) {
        ...
        if (this._oasisOverlayLoadHandler) {
          overlayBrowser.removeEventListener(
            "load",
            this._oasisOverlayLoadHandler
          );
          delete this._oasisOverlayLoadHandler;
        }
      } else {
        // Temporary hide: keep iframe loaded to preserve chat state.
        overlay.hidden = true;
        // Keep the oasis-assistant attribute removed to indicate overlay is not visible
        document.documentElement.removeAttribute("oasis-assistant-overlay");
      }
