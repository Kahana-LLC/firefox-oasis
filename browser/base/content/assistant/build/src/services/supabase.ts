...
export default class SupabaseAuth {
  ...
  private setFirefoxOAuthMarker(
    ...
    const writeCookie = (baseUrl: string) => {
      try {
        ...
        logWarn("Failed to set Firefox OAuth marker cookie:", e);
      }
    };
  ...
  public async signInWithGoogle(
    ...
    try {
      ...
      logDebug(`[Oasis OAuth][${flowId}] Preparing Google OAuth launcher URL`);
      ...
      logDebug(`[Oasis OAuth][${flowId}] Generated launcher URL:`, launcherUrl);
      return {
        user: null,
  ...
  public async signInWithAzure(
    ...
    try {
      ...
      const { flowId, launcherUrl } = launch;
      logDebug(`[Oasis OAuth][${flowId}] Preparing Azure OAuth launcher URL`);
      ...
      logError("Azure sign in error:", error);
      return { user: null, error: error as AuthError };
    }
  }
  ...
  public async signInWithApple(
    ...
    try {
      ...
      const { flowId, launcherUrl } = launch;
      logDebug(`[Oasis OAuth][${flowId}] Preparing Apple OAuth launcher URL`);
  ...
  public async handleOAuthCallbackData(
    ...
    try {
      logDebug(`[Oasis OAuth][${flowId}] Handling callback data:`, authData);
      ...
      if (accessToken && refreshToken) {
        logDebug(`[Oasis OAuth][${flowId}] Setting session from tokens...`);
      ...
      logWarn(`[Oasis OAuth][${flowId}] No valid OAuth data found`);
      this.clearActiveOAuthLaunch(flowId);
      return { success: false, error: "No valid OAuth data" };
    } catch (error) {
      logError(`[Oasis OAuth][${flowId}] Error handling callback data:`, error);
      this.clearActiveOAuthLaunch(flowId);
      return {
        success: false,
