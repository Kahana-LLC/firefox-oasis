...
class LocalMemoryService {
  private dbPromise: Promise<IDBPDatabase<MemoryDB> | null>;
  ...
  private createDbPromise(): Promise<IDBPDatabase<MemoryDB> | null> {
    if (typeof indexedDB === "undefined") {
      return Promise.resolve(null);
    }

    try {
      return openDB<MemoryDB>("oasis-memory", 2, {
        ...
        console.warn("[LocalMemory] IndexedDB unavailable:", error);
        return null;
      });
    } catch (error) {
      console.warn("[LocalMemory] IndexedDB unavailable:", error);
    ...
    const results = this.miniSearch.search(query, {
      filter: result => {
        if (filter?.hub) {
          ...
          return (
            (result.metadata?.hubName || "").toLowerCase() ===
            filter.hub.toLowerCase()
          );
        }
        return true;
      },
