...
import {
  searchHistoryByKeywordHybrid,
  type KeywordHistoryResult,
} from "./historyKeywordSearch.js";
import type { HistorySearchMode } from "../utils/historySearchQuery.js";

const MAX_HISTORY_ENTRIES = 500;
const KEYWORD_AUTO_MIN_RESULTS = 2;
...
function buildEmbeddingText(
  ...
  try {
    ...
    return `${title} ${domain} ${pathWords} ${searchParams}`
      .replace(/\s+/g, " ")
      .trim();
  } catch {
    return `${title} ${url}`;
...
class SemanticHistorySearch {
  ...
  async search(
    query: string,
    limit = 10,
    options: { mode?: HistorySearchMode } = {}
  ...
  private async searchSemanticSafe(
    query: string,
    limit: number
  ): Promise<SearchResult[]> {
    try {
...
function keywordResultsToSearchResults(
  results: KeywordHistoryResult[]
): SearchResult[] {
