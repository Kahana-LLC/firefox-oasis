...
class SemanticHistorySearch {
  ...
  async search(query: string, limit = 5): Promise<SearchResult[]> {
    ...
    let results = await historyVectorStore.search(queryEmbedding, limit, 0.35);
    if (results.length === 0) {
      results = await historyVectorStore.search(queryEmbedding, limit, 0.2);
    }
    if (results.length === 0) {
      results = await historyVectorStore.search(queryEmbedding, limit, 0.12);
    }
    if (results.length === 0) {
