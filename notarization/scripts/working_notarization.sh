...
# Working Notarization Solution
# This script creates a properly notarized app using the correct approach
...
SIGNED_APP_PATH="$BUILD_DIR/Oasis-Notarized.app"
...
echo -e "${GREEN}🔐 Working Notarization Solution...${NC}"
...
find "$SIGNED_APP_PATH" -type d -empty -delete

# Create a proper entitlements file for notarization
echo -e "${YELLOW}📝 Creating proper entitlements...${NC}"
cat > "notarization_entitlements.plist" << EOF
...
# Sign using the proper approach - sign everything at once
echo -e "${YELLOW}🔏 Signing app with proper approach...${NC}"
...
find "$SIGNED_APP_PATH" -name "*.app" -not -path "*/Oasis-Notarized.app" | while read -r app_bundle; do
    codesign --force --sign "$DEVELOPER_ID" "$app_bundle" 2>/dev/null || true
done

# Sign the main app bundle
echo -e "${BLUE}🔏 Signing main app bundle...${NC}"
codesign --force --sign "$DEVELOPER_ID" --options runtime --entitlements "notarization_entitlements.plist" "$SIGNED_APP_PATH"
