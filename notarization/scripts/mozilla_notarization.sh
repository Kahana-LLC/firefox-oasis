...
# Mozilla-Style Notarization
# This script uses Mozilla's built-in signing system for proper notarization
...
SIGNED_APP_PATH="$BUILD_DIR/Oasis-Mozilla-Signed.app"
DMG_NAME="Oasis-Browser-${APP_VERSION}-Mozilla-Signed-$(date +%Y%m%d)"
...
echo -e "${GREEN}🔐 Using Mozilla's Signing System for Notarization...${NC}"
...
if [ ! -f "mach" ]; then
    echo -e "${RED}❌ Error: mach command not found!${NC}"
    echo "Please run this script from the Firefox source root directory"
    exit 1
fi
...
# Use Mozilla's built-in signing system
echo -e "${YELLOW}🔏 Using Mozilla's macos-sign command...${NC}"
echo -e "${BLUE}This will use Mozilla's sophisticated signing configuration${NC}"

# First, let's try with developer entitlements (simpler)
echo -e "${YELLOW}🔏 Signing with developer entitlements...${NC}"
./mach macos-sign \
    --app-path "$SIGNED_APP_PATH" \
...
echo -e "${GREEN}✅ Mozilla-style signing completed!${NC}"

# Create a ZIP file for notarization
echo -e "${YELLOW}📦 Creating ZIP file for notarization...${NC}"
ZIP_PATH="${APP_NAME}-${APP_VERSION}-mozilla-for-notarization.zip"
rm -f "$ZIP_PATH"
...
echo -e "${GREEN}🎉 SUCCESS! Mozilla-style notarized DMG created!${NC}"
