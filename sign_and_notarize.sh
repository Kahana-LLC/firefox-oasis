...
# Oasis Browser Code Signing and Notarization Script
# This script signs the Oasis Browser app and notarizes it with Apple
...
echo -e "${GREEN}🔐 Starting Code Signing and Notarization Process...${NC}"
...
echo -e "${YELLOW}🔍 Checking signing certificate...${NC}"
if ! security find-identity -v -p codesigning | grep -q "$DEVELOPER_ID"; then
    echo -e "${RED}❌ Error: Developer ID certificate not found!${NC}"
    echo "Please ensure your Developer ID Application certificate is installed in Keychain."
    exit 1
fi
...
sign_directory() {
    local dir="$1"
    echo -e "${YELLOW}📁 Signing directory: $dir${NC}"
    
    # Sign all files first
    find "$dir" -type f \( -name "*.dylib" -o -name "*.so" -o -name "*.framework" -o -name "*.app" \) | while read -r file; do
        sign_file "$file"
    done
    
    # Sign executables
    find "$dir" -type f -perm +111 -not -name "*.dylib" -not -name "*.so" | while read -r file; do
...
# Sign all components
echo -e "${YELLOW}🔏 Signing Oasis Browser components...${NC}"

# Sign all dylibs and frameworks first
sign_directory "$SIGNED_APP_PATH/Contents/Frameworks"
sign_directory "$SIGNED_APP_PATH/Contents/MacOS"
