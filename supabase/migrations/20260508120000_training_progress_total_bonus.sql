-- Extend training gallery RPC with lifetime bonus tokens from feedback_token_grants.
...
BEGIN
  uid := auth.uid();
  IF uid IS NULL THEN
    RETURN QUERY SELECT 0::bigint, 0, 0, NULL::date, 0::bigint;
    RETURN;
  END IF;
  ...
  SELECT coalesce(sum(g.tokens), 0)::bigint
  INTO bonus
  FROM public.feedback_token_grants g
  WHERE g.user_id = uid;

  IF tot IS NULL OR tot = 0 THEN
    RETURN QUERY SELECT 0::bigint, 0, 0, NULL::date, coalesce(bonus, 0)::bigint;
    RETURN;
  END IF;
  ...
  RETURN QUERY SELECT tot, streak, lng, last_d, coalesce(bonus, 0)::bigint;
END;
$$;
